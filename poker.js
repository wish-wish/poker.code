	...
	if (window.CanvasRenderingContext2D) {
		...
		CanvasRenderingContext2D.prototype.drawPokerCard = function(x, y, size, suit, point) {
			...
			if (size >= 180) {
				if(point=='o')
				{
					let w=20;
					let h=30;
					this.fillPokerSymbol(ax(w/2), ay(h/2), as(w), 'o');
					this.fillPokerSymbol(ax(size*0.75-w/2), ay(size-h/2), as(-w), 'o');
					
					if (suit === 'h' || suit === 'd') {
						this.drawPokerCrown(ax(w*2), ay(h/2), as(h), '#b55', '#a22');
						this.drawPokerCrown(ax(w*2), ay(h/2), as(h), '#fdf98b', '#e7bd4f', '#a22');
						// this.drawPokerCrown(ax(size*0.6-w/2), ay(size-h/2), as(-h), '#b55', '#a22');
						// this.drawPokerCrown(ax(size*0.6-w/2), ay(size-h/2), as(-h), '#fdf98b', '#e7bd4f', '#a22');
					} else {
						this.drawPokerCrown(ax(w*2), ay(h/2), as(h), '#000', '#000');
						this.drawPokerCrown(ax(w*2), ay(h/2), as(h), '#eee', '#888', '#333');
						// this.drawPokerCrown(ax(size*0.75-w/2-h), ay(size-w/2), as(-h), '#000', '#000');
						// this.drawPokerCrown(ax(size*0.75-w/2-h), ay(size-h-w/2), as(-h), '#eee', '#888', '#333');
					}					
					// var img=new Image();
					// img.src="./zd/j_"+suit+"o.png";
					// var that=this;
					// img.onload=function()
					// {
					// 	let width=img.width;
					...
					// 	ctx.drawImage(img, ax(w*1.7),ay(h*2), as(width/2.7), as(height/2.7));
					// }
					var img=new Image();
					img.src="./zd/joker_a"+suit+"o.png";
					var that=this;
					img.onload=function()
					{
						let width=img.width;
						let height=img.height;
						var ctx=that.canvas.getContext('2d');
						ctx.drawImage(img, ax(w*1.5),ay(h*2), as(width/3), as(height/3));
					}					
				}else
				{
					let w=20;
					...
					{
						var img=new Image();
						img.src="./zd/human_"+suit+"k.png";	
						var that=this;
						img.onload=function()
						{
							var ctx=that.canvas.getContext('2d');
							ctx.drawImage(img, ax(w*1.5), ay(h/3+3), as(img.width/2.5), as(img.height/2.5));
							//TODO:can't rotate 
							ctx.drawImage(img, ax(w*1.5), ay(img.width/2.5+h/3-3), as(img.width/2.5), as(img.height/2.5));
						}						
					}
					else if(point=='q')
					{
						var img=new Image();
						img.src="./zd/human_"+suit+"q.png";	
						var that=this;						
						img.onload=function()
						{
							var ctx=that.canvas.getContext('2d');
					...
					{						
						...
						img.onload=function()
						{
							...
							let height=img.height;
							var ctx=that.canvas.getContext('2d');							
							ctx.drawImage(img, ax(w*1.2),ay(h), as(width/1.2), as(height/1.2));							
						}
						// that.canvas.toBlob((blob)=>{
						// 	console.log(blob);
						// },'image/png',1);
						
						//var elem=document.body.appendChild(img);
					...
					else if(point=='0')
					{
						w=24;
						h=32;
						this.fillPokerSymbol(ax((size*0.75-2.4*h)/2), ay((size-5.0*h)/2), as(h), suit);
						this.fillPokerSymbol(ax((size*0.75-2.4*h)/2), ay((size-2.0*h)/2), as(h), suit);
						this.fillPokerSymbol(ax((size*0.75-0.4*h)/2), ay((size+2.0*h)/2), as(-h), suit);
						this.fillPokerSymbol(ax((size*0.75-0.4*h)/2), ay((size+5.0*h)/2), as(-h), suit);
						...
						this.fillPokerSymbol(ax((size*0.75-h)/2), ay((size-3.5*h)/2), as(h), suit);
						this.fillPokerSymbol(ax((size*0.75+h)/2), ay((size+3.5*h)/2), as(-h), suit);
					}
					else if(point=='9')
					{
						...
						h=32;
						this.fillPokerSymbol(ax((size*0.75-2.4*h)/2), ay((size-5.0*h)/2), as(h), suit);
						this.fillPokerSymbol(ax((size*0.75-2.4*h)/2), ay((size-2.7*h)/2), as(h), suit);
						this.fillPokerSymbol(ax((size*0.75-0.4*h)/2), ay((size+2.7*h)/2), as(-h), suit);
						this.fillPokerSymbol(ax((size*0.75-0.4*h)/2), ay((size+5.0*h)/2), as(-h), suit);
						...
						this.fillPokerSymbol(ax((size*0.75-h)/2), ay((size-h)/2), as(h), suit);
					}
					else if(point=='8')
					{
						w=27;
						h=36;
						this.fillPokerSymbol(ax((size*0.75-2.3*h)/2), ay((size-4*h)/2), as(h), suit);
						this.fillPokerSymbol(ax((size*0.75-2.3*h)/2), ay((size-h)/2), as(h), suit);
						this.fillPokerSymbol(ax((size*0.75-0.3*h)/2), ay((size+4*h)/2), as(-h), suit);
						this.fillPokerSymbol(ax((size*0.75+0.3*h)/2), ay((size-4*h)/2), as(h), suit);
						this.fillPokerSymbol(ax((size*0.75+0.3*h)/2), ay((size-h)/2), as(h), suit);
						this.fillPokerSymbol(ax((size*0.75+2.3*h)/2), ay((size+4*h)/2), as(-h), suit);

						this.fillPokerSymbol(ax((size*0.75-h)/2), ay((size-2.5*h)/2), as(h), suit);
						this.fillPokerSymbol(ax((size*0.75+h)/2), ay((size+2.5*h)/2), as(-h), suit);
					}
					else if(point=='7')
					{
					...
					else if(point=='6')
					{
						w=27;
						h=36;
						this.fillPokerSymbol(ax((size*0.75-2.2*h)/2), ay((size-4*h)/2), as(h), suit);
						this.fillPokerSymbol(ax((size*0.75-2.2*h)/2), ay((size-h)/2), as(h), suit);
						this.fillPokerSymbol(ax((size*0.75-0.2*h)/2), ay((size+4*h)/2), as(-h), suit);
						this.fillPokerSymbol(ax((size*0.75+0.2*h)/2), ay((size-4*h)/2), as(h), suit);
						this.fillPokerSymbol(ax((size*0.75+0.2*h)/2), ay((size-h)/2), as(h), suit);
						this.fillPokerSymbol(ax((size*0.75+2.2*h)/2), ay((size+4*h)/2), as(-h), suit);
					...
					else if(point=='3')
					{
						w=30;
						h=40;
						this.fillPokerSymbol(ax((size*0.75-h)/2), ay((size-4*h)/2), as(h), suit);
						this.fillPokerSymbol(ax((size*0.75-h)/2), ay((size-h)/2), as(h), suit);
						this.fillPokerSymbol(ax((size*0.75+h)/2), ay((size+4*h)/2), as(-h), suit);
					}
					else if(point=='2')
					{
						w=45;
						h=60;
						this.fillPokerSymbol(ax((size*0.75-h)/2), ay((size-3*h)/2), as(h), suit);
						this.fillPokerSymbol(ax((size*0.75+h)/2), ay((size+3*h)/2), as(-h), suit);
					}
					w=20;
					h=30;
					this.fillPokerSymbol(ax(w/2), ay(w/2), as(h), point);
					this.fillPokerSymbol(ax(w/2), ay(w*2), as(w), suit);
					this.fillPokerSymbol(ax(size*0.75-w/2), ay(size-w/2), as(-h), point);
					this.fillPokerSymbol(ax(size*0.75-w/2), ay(size-h-w/2), as(-w), suit);						
				}								
			}	
			else if (size >= 100) {
				if (point !== 'o') {
		...
		CanvasRenderingContext2D.prototype.drawPokerBack = function(x, y, size, foregroundColor, backgroundColor) {
			...
			img.onload=function()
			{
				...
				let height=img.height;
				var ctx=that.canvas.getContext('2d');
				ctx.drawImage(img, ax((size*0.75-h)/2),ay((size-h)/2), as(h), as(h));
			}
		...
		CanvasRenderingContext2D.prototype.svgCurve = function(x, y, size, svgPath) {
			...
			for (pathNumber in svgPathArray) {
				...
				if (pathArray[0] === 'M') {
					...
					//control point 1, control point2, end
